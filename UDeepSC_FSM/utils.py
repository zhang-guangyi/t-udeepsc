...
def load_checkpoint(model,args):
    ...
    for key in all_keys:
        ...
        # elif 'mask_generator' in key or 'FSM' in key:
        #     continue
        else:
...
def adjust_learning_rate(param_groups, init_lr, min_lr, step, max_step, warming_up_step=2, warmup_predictor=False, base_multi=0.1):
    ...
    for param_group in param_groups:
        if param_group['name'] == 'mask_generator':
...
def get_param_groups(model, weight_decay):
    ...
    for name, param in model.named_parameters():
        if 'mask_generator' in name:
            ...
            decay.append(param)
    return [
        {'params': mask_generator, 'weight_decay': weight_decay, 'name': 'predictor'},
...
class DiffPruningLoss(torch.nn.Module):
    def __init__(self, base_criterion: torch.nn.Module, dynamic=True, ratio_weight=2.0, main_weight=1.):
        ...
        self.main_weight = 1.
        self.surp_weight = 120
        self.rho_weight = 0
        self.mask_weight = 0.0
        self.codebook_weight = 2.0    
    ...
    def forward(self, outputs, labels, ta_perform):
        ...
        surp_loss = 0.0
        mask_loss = 0.0
        keep_ratio_text = np.array([0.])
        keep_ratio_img = np.array([0.])
        rho_img, rho_text = 0., 0.
        ...
        #     m_group = (m_dict['img'], m_dict['text'])
        #     rho = (rho['img'], rho['text'])

        # if ta_perform.startswith('img'):
        #     codebook_loss = codebook_loss_dict['img']
        # elif ta_perform.startswith('text'):
        #     codebook_loss = codebook_loss_dict['text']
        # elif ta_perform.startswith('vqa'):
        #     codebook_loss = codebook_loss_dict['img'] + codebook_loss_dict['text'] * 0.8
        # elif ta_perform.startswith('msa'):
        #     codebook_loss = codebook_loss_dict['img'] + codebook_loss_dict['text'] * 0.1 + codebook_loss_dict['spe']
        ...
        if ta_perform.startswith('vqa'):
            for i, mask_m in enumerate(m_dict['text']):
                ...
                surp_loss = surp_loss + ((keep_ratio_text - rho_dict['text'][i]) ** 2).mean()    ### The supervised loss.  5   3 
                mask_loss =  mask_loss + keep_ratio_text.mean()
            for i, mask_m in enumerate(m_dict['img']):
                mask_m = mask_m.squeeze(2)
                keep_ratio_img = mask_m.mean(1)
                surp_loss = surp_loss + ((keep_ratio_img - rho_dict['img'][i]) ** 2).mean()    ### The supervised loss.  5   3 
                mask_loss =  mask_loss + keep_ratio_img.mean()
            ...
            loss = self.main_weight * main_loss + \
                ...
                # self.rho_weight * (rho_dict['img'][-1] * 6 + rho_dict['text'][-1]) + \
            ...
            loss = self.main_weight * main_loss + \
                self.surp_weight  * surp_loss + \
                self.codebook_weight * codebook_loss 
                # self.rho_weight * rho_dict['img'][-1] * 30 + \
                # self.mask_weight * mask_loss
            ...
            loss = self.main_weight * main_loss + \
                self.surp_weight * surp_loss + \
                self.codebook_weight * codebook_loss  
                # self.rho_weight * rho_dict['text'][-1] * 0.4 + \
                # self.mask_weight * mask_loss
